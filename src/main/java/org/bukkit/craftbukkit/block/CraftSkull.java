...
public class CraftSkull extends CraftBlockEntityState<SkullBlockEntity> implements Skull {
    ...
    public boolean setOwner(String name) {
        ...
        if (profile == null) {
            profile = CraftServer.INSTANCE.getServer().services().nameToIdCache().get(name).map(nameAndId -> (((NameAndIdBridge)(Object)nameAndId).cardboard$toUncompletedGameProfile())).orElse(null);
        }
        if (profile == null) {
            return false;
        }
