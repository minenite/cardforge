...
public class CraftScheduler implements BukkitScheduler {
    ...
    public void cancelTask(final int taskId) {
        ...
        task = new BukkitTaskImpl(
                new Runnable() {
                    @Override
                    public void run() {
                        if (!check(CraftScheduler.this.temp)) check(CraftScheduler.this.pending);
                    }
    ...
    void parsePending() {
        ...
        // We split this because of the way things are ordered for all of the async calls in CraftScheduler (it prevents race-conditions)
