...
public class CraftVillagerZombie extends CraftZombie implements ZombieVillager {
    ...
    public void setConversionTime(int time, boolean broadcastEntityEvent) {
        ...
        if (time < 0) {
            ...
            this.getHandle().removeEffect(MobEffects.STRENGTH);//, org.bukkit.event.entity.EntityPotionEffectEvent.Cause.CONVERSION); // TODO?
        } else {
            this.getHandle().startConverting(null, time);//, broadcastEntityEvent); // Paper - missing entity behaviour api - converting without entity event // TODO?
        }
    }
