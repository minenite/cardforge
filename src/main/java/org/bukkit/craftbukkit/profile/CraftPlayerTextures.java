...
public final class CraftPlayerTextures implements PlayerTextures {
	...
	private static URL parseUrl(@Nullable String urlString) {
		if (urlString == null) {
			...
			} catch (MalformedURLException | IllegalArgumentException var2) {
				return null;
			}
		}
	}
	...
	public CraftPlayerTextures(SharedPlayerProfile profile) {
		...
		if (this.dirty) {
			...
			if (this.isEmpty()) {
				...
				if (this.skin != null) {
					...
					JsonObject skinTexture = JsonHelper.getOrCreateObject(texturesMap, Type.SKIN.name());
				...
				if (this.cape != null) {
					JsonObject texturesMap = JsonHelper.getOrCreateObject(propertyData, "textures");
					JsonObject skinTexture = JsonHelper.getOrCreateObject(texturesMap, Type.CAPE.name());
	...
	public boolean equals(Object obj) {
		if (this == obj) {
			return true;
		} else if (!(obj instanceof CraftPlayerTextures other)) {
			return false;
		} else {
			Property property = this.getProperty();
